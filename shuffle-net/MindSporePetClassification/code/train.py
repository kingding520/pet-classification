...
if __name__ == '__main__':
    ...
    load_ckpt(
        backbone_net,
        args_opt.pretrain_ckpt,
        trainable=False,
        filter_head=getattr(args_opt, 'filter_head', False),
        strict_load=False,
    )
    ...
    # catch backbone features (cache is isolated by network name)
    data, step_size = extract_features(backbone_net, args_opt.dataset_path, config, feature_tag=config.network)
    ...
    export_mindir(net, config.network)
