...
def extract_features(net, dataset_path, config, feature_tag=None):
    print("start cache feature!")
    tag = feature_tag or getattr(config, 'network', None) or 'default'
    features_folder = os.path.join(dataset_path, f"features_{tag}")
