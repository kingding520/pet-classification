...
from .shufflenetv2 import ShuffleNetV2Backbone, ShuffleNetV2Head, shufflenet_v2
...
def _filter_ckpt_params(param_dict, exclude_substrings):
    if not exclude_substrings:
        return param_dict
    filtered = {}
    for k, v in param_dict.items():
        if any(s in k for s in exclude_substrings):
            continue
        filtered[k] = v
    return filtered


def load_ckpt(network, pretrain_ckpt_path, trainable=True, filter_head=False, strict_load=False):
    """Load checkpoint into network.

    - strict_load=False: ignore unmatched parameters (recommended for fine-tune)
    - filter_head=True: drop common classifier/head parameters from checkpoint
    """
    param_dict = load_checkpoint(pretrain_ckpt_path)
    if filter_head:
        param_dict = _filter_ckpt_params(
            param_dict,
            exclude_substrings=(
                'head', 'classifier', 'fc', 'dense',
            ),
        )
...
def define_net(config, activation="None"):
    network_name = getattr(config, 'network', 'mobilenetv2').lower()
    if network_name == 'mobilenetv2':
        ...
        backbone_net = ShuffleNetV2Backbone(model_size='1.0x')
        head_net = ShuffleNetV2Head(input_channel=backbone_net.out_channels,
                                    num_classes=config.num_classes)
        net = shufflenet_v2(backbone_net, head_net, activation)
    else:
...
def train(network, model, net, data, config):
    ...
    for epoch in range(config.epoch_size):
        ...
        if acc_f >= best_acc["acc"]:
            ...
            net_name = getattr(config, 'network', 'mobilenetv2')
            bast_acc_path = os.path.join(save_ckpt_path, f"{net_name}_best.ckpt")
        ...
        if epoch - best_acc["epoch"] > 10:  
            ...
            net_name = getattr(config, 'network', 'mobilenetv2')
            load_ckpt(net, os.path.join(save_ckpt_path, f"{net_name}_best.ckpt"))
            break
