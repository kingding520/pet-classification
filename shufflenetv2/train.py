...
# ============================================================================
"""Train ShuffleNetV2 on PetImages (cat vs dog)."""
import argparse
import ast
import os
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_path', type=str, default='dataset\\PetImages', help='Dataset root path')
    parser.add_argument('--resume', type=str, default='', help='resume training with existed checkpoint')
    parser.add_argument('--is_distributed', type=ast.literal_eval, default=False, help='distributed training')
    parser.add_argument('--platform', type=str, default='CPU', choices=('CPU', 'Ascend', 'GPU'), help='run platform')
    ...
    # dataloader
    dataset = create_dataset(args_opt.dataset_path, True, cfg.rank, cfg.group_size, cfg)
    ...
    optimizer = Momentum(params=group_params, learning_rate=lr, momentum=cfg.momentum)

    if args_opt.resume:
        ckpt = load_checkpoint(args_opt.resume)
        load_param_into_net(net, ckpt)
    model = Model(net, loss_fn=loss, optimizer=optimizer, metrics={'acc': nn.Top1CategoricalAccuracy()})
    ...
    if args_opt.is_distributed and cfg.is_save_on_master:
        ...
        model.train(cfg.epoch_size, dataset, callbacks=callbacks, dataset_sink_mode=dataset_sink_mode)
    else:
        callbacks.append(ckpoint_cb)
        model.train(cfg.epoch_size, dataset, callbacks=callbacks, dataset_sink_mode=dataset_sink_mode)
    print("train success")
