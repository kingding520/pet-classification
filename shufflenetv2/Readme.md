...
Dataset used: PetImages (cat vs dog, 2 classes).

- Directory structure (ImageFolder):
  - `dataset/PetImages/train/cat/*.jpg`
  - `dataset/PetImages/train/dog/*.jpg`
  - `dataset/PetImages/eval/cat/*.jpg`
  - `dataset/PetImages/eval/dog/*.jpg`
...
# training example
  python:
      GPU: mpirun --allow-run-as-root -n 8 python train.py --is_distributed=True --platform='GPU' --dataset_path='dataset/PetImages' > train.log 2>&1 &

  shell:
      GPU: cd scripts & sh run_distribute_train_for_gpu.sh 8 0,1,2,3,4,5,6,7 dataset/PetImages
```
...
# infer example
  python:
      GPU: CUDA_VISIBLE_DEVICES=0 python eval.py --platform='GPU' --dataset_path='dataset/PetImages' --checkpoint='checkpoint_file' > eval.log 2>&1 &

  shell:
      GPU: cd scripts & sh run_eval_for_gpu.sh 'dataset/PetImages' 'checkpoint_file' 
```
